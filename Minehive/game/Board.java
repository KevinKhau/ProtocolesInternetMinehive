...
public class Board {
	...
	public boolean isBombFrom(byte square) {
		return (square & MINE_BIT) != 0;
	}

	/**
	 * Renvoie le numéro de la case, indépendamment des autres bits (mine,
	 * caché, etc.)
	 * 
	 * @param x
	 * @param y
	...
	/**
	 * Obtenir la valeur d'une case case, qu'elle soit cachée ou non. S'il y a
	 * une mine, renvoie -1.
	 */
	...
	public synchronized void updateValueAt(int x, int y, int content) {
		if (content < MIN_VALUE || content > MAX_VALUE) {
			...
			throw new ArrayIndexOutOfBoundsException(
					"Valeur de case invalide. Minimum : " + MIN_VALUE + ", Maximum : " + MAX_VALUE + ".");
		}
		if (content == -1) {
			board[x + y * width] = MINE_BIT;
		} else {
			/* Update number */
	...
	public LinkedList<String> lineContentAt(int ordinate) {
	...
	private int getSafeSquaresCount() {
		return board.length - nbMines;
	}

	/** Returns true if the game has ended */
	public boolean isFinished() {
		return nbRevealed == getSafeSquaresCount();
	}
	
	private synchronized void incNbRevealed() {
		nbRevealed++;
	}
	...
	public int getCompletion() {
		int completion = 0;
		try {
			completion = (nbRevealed * 100) / getSafeSquaresCount();
	...
	private void initBombs() {
		...
		/*
		 * int bombs = 0; while (bombs <= 5) { bombs = rand.nextInt(board.length
		 * / 5); }
		 */
		int bombs = nbMines;
	...
	private void placeMine(int position, int x, int y) {
		...
		board[position] = MINE_BIT | HIDDEN_BIT;
	...
	/**
	 * Retourne les points du joueur à ajouter ou retirer lors d'un clic
	 * 
	...
	public List<String[]> clickAt(int x, int y, String user) {
		...
		if ((board[position] & MINE_BIT) != 0) { // It's a bomb !
	...
	private void moveMine(int x, int y) {
		...
		// Left
		if (x > 0) {
			if ((board[position - 1] & MINE_BIT) == 0)
				board[position - 1]--;
			else
				board[position]++;
		}
		...
		// Top Left
		if (x > 0 && y > 0) {
			if ((board[position - 1 - width] & MINE_BIT) == 0)
				board[position - 1 - width]--;
			else
				board[position]++;
		}
		...
		// Up
		if (y > 0) {
			if ((board[position - width] & MINE_BIT) == 0)
				board[position - width]--;
			else
				board[position]++;
		}
		...
		/*
		 * We need to add this mine elsewhere to have the same amount of mines
		 * as nb_bombs
		 */
		Random rand = new Random();
		int b_x, b_y, b_position;
		do {
			b_x = rand.nextInt(width);
			b_y = rand.nextInt(height);
			b_position = b_x + b_y * width;
		/* Have to be a different square, with no mine */
		} while (isMineAt(b_x, b_y) || position == b_position);
		// Place the new mine
		placeMine(b_position, b_x, b_y);
	}

	/** Révèle une case safe */
	...
	public void revealAll() {
		for (int i = 0; i < board.length; i++) {
			setVisible(i);
			nbRevealed = getSafeSquaresCount();
		}
	}
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				...
				if ((board[j + i * width] & HIDDEN_BIT) == 0) {
					if ((board[j + i * width] & MINE_BIT) != 0) {
						gc.setFill(Color.BLACK);
						gc.fillText("¤", j * (tile + gap) + (tile + gap) / 2, i * (tile + gap) + (tile + gap) / 2,
								tile);
					} else {
						if ((board[j + i * width] & VALUE_MASK) != 0) {
							...
							gc.fillText(Integer.toString(val), j * (tile + gap) + (tile + gap) / 2,
									i * (tile + gap) + (tile + gap) / 2, tile);
						}
					}
				}
	...
	/**
	 * Affiche le plateau sans dévoiler les cases, tel qu'un utilisateur devrait
	 * le voir.
	 */
	public void display() {
		for (int i = 0; i < height; i++) {
				...
				if ((board[j + i * width] & HIDDEN_BIT) != 0) {
					...
					if ((board[j + i * width] & MINE_BIT) != 0) {
						...
						if ((board[j + i * width] & VALUE_MASK) == 0) {
							...
							/*
							 * UNIX only, colored output in terminal int val = board[j + i * width] &
							 * VALUE_MASK; switch (val) { case 1: System.out.print("\u001B[34m"); //
							 * Blue break; case 2: System.out.print("\u001B[32m"); // Green break; case
							 * 3: System.out.print("\u001B[31m"); // Red break; case 4:
							 * System.out.print("\u001B[35m"); // Dark blue break; case 5:
							 * System.out.print("\u001B[35m"); // Dark red break; case 6:
							 * System.out.print("\u001B[36m"); // Teal break; case 7:
							 * System.out.print(""); // Black break; case 8: System.out.print(""); //
							 * Grey break; } System.out.print((board[j + i * width] & VALUE_MASK) +
							 * "\u001B[0m");
							 */
	...
	/*
	 * public static void main(String[] args) { boolean quit = false; Board b =
	 * new Board(); Scanner scanner = new Scanner(System.in);
	 * 
	 * while(!quit) { b.reset(); boolean game_over = false;
	 * System.out.println("New game started");
	 * 
	 * while(!game_over) { b.display();
	 * 
	 * System.out.println("enter x: "); int x = scanner.nextInt();
	 * 
	 * System.out.println("enter y: "); int y = scanner.nextInt();
	 * 
	 * if (x == -1 || y == -1) b.revealAll(); else b.clickAt(x, y); } }
	 * 
	 * scanner.close(); }
	 */
